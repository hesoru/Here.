...
public class AttendanceApp {
    ...
    private List<Child> childRegistry;
    private List<Caregiver> caregiverRegistry;
    private List<Child> notCheckedIn;
    private List<Child> checkedIn;
    private List<Child> checkedOut;
    ...
    // REQUIRES: User input must be integer from 1 to 9.
    ...
    // REQUIRES: childFullName is first and last name separated by a space (case-sensitive). If caregiver does not exist
    //           in caregiverRegistry (null), caregiverFullName is first and last name separated by a space
    //           (case-sensitive), caregiverPhoneNum is 10 digits without spaces, and caregiverEmail is not an empty
    //           string.
    ...
    private void addNewChildToRegistry() {
        ...
        if (selectCaregiverInRegistry(primaryCaregiverFullName) != null) {
            ...
            System.out.println(child.getChildFullName() + " is not checked in.\n");
        } else {
            caregiverIsNull();
    ...
    // EFFECTS: Searches child registry for child with full name matching the user input (case-sensitive). Returns
    ...
    // REQUIRES: childFullName and authorizedCaregiverFullName are first and last name separated by a space
    ...
    //          and adds all children in the childRegistry to the notCheckedIn list.
    ...
    //          them to the checkedIn list, and sets the child's checkout time to the current time. Otherwise, prints
    //          that the child was not found in list of children not yet checked in.
    private void checkInChild() {
        Child childToCheckIn = selectChildInRegistry();
        String childFullName = childToCheckIn.getChildFullName();
        ...
        for (Child c : notCheckedIn) {
            if (childToCheckIn == c) {
                ...
                System.out.println(childFullName + " checked in at " + c.getCheckInTime() + ".\n");
            }
        }
        if (!childFound) {
    ...
    //          and prints primary caregiver's full name, phone number, and email.
    ...
    //          prints information about caregivers authorized to pick up the selected child (full name, phone number,
    //          and email).
    private void viewAuthorizedToPickUp() {
        Child child = selectChildInRegistry();

        System.out.println("Caregivers authorized to pick up " + child.getChildFullName() + ":" + "\n");
    ...
    // REQUIRES: childToCheckOut exists in childRegistry (!null).
    ...
    // REQUIRES: child exists in childRegistry (!null).
    ...
    //          if the child checked in today (prints child, check out time, and caregiver that picked up the child).
    //          If the caregiver entered does not exist, then the method directs the user to add a new caregiver and
    //          then returns the user to the beginning of the method. Prints if the caregiver is not authorized to pick
    //          up the child or if the child has not checked in today.
    private void checkOutChild() {
        Child childToCheckOut = selectChildInRegistry();
        String childFullName = childToCheckOut.getChildFullName();
        ...
        if (caregiverToPickUp == null) {
            ...
            checkOutChild();
        } else if (isAuthorizedToPickUp(caregiverToPickUp, childToCheckOut) && childIsCheckedIn(childToCheckOut)) {
            checkedIn.remove(childToCheckOut);
            checkedOut.add(childToCheckOut);
            childToCheckOut.setCheckOutTime(LocalTime.now());
            System.out.println(childFullName + " was checked out at " + childToCheckOut.getCheckOutTime() + " by "
                    + caregiverToPickUp.getCaregiverFullName() + ".\n");
        } else {
    ...
    // MODIFIES: this, Caregiver
    // EFFECTS: Searches caregiver registry for caregiver with full name matching the user input (case-sensitive).
    //          Removes caregiver from registry (prints that caregiver was removed from the registry) if not a primary
    //          caregiver for an existing child.
    private void removeCaregiverFromRegistry() {
        ...
        String caregiverFullName = scanner.nextLine();
        Caregiver caregiver = selectCaregiverInRegistry(caregiverFullName);

        boolean isPrimaryCaregiver = false;
        for (Child c : childRegistry) {
            if (c.getPrimaryCaregiver() == caregiver) {
                isPrimaryCaregiver = true;
                System.out.println(caregiverFullName + " is the primary caregiver listed for " + c.getChildFullName()
                        + ".\n" + "Remove this child before removing their caregiver.\n");
            }
        }

        if (!isPrimaryCaregiver) {
            System.out.println("Are you sure you want to remove " + caregiverFullName
                    + " from the caregiver registry?");
    ...
    // MODIFIES: this, Child
    // EFFECTS: Searches child registry for child with full name matching the user input (case-sensitive), then
    //          removes child from registry (prints that child was removed from the registry).
